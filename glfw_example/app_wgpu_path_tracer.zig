...
const App = struct {
    ...
    pub fn init(allocator: std.mem.Allocator) !Self {
        ...
        path_tracer.setResolution(ornament.Resolution{ .width = app_config.WIDTH, .height = app_config.HEIGHT });

        const target_buffer = try path_tracer.getOrCreateTargetBuffer();
        const viewport = try Viewport.init(&path_tracer.device_state, path_tracer.state.getResolution(), &target_buffer.buffer);
        return .{
            .allocator = allocator,
            .window = window,
